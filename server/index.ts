...
if (process.env.NODE_ENV !== 'production') mongoose.connect(config.default.db.uri, config.default.db.options);
else mongoose.connect(config.default.db.uri);
...
if (process.env.NODE_ENV === 'production') app.listen(process.env.PORT || 5001);
else
