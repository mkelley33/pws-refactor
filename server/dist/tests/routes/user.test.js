    ...
    describe('# POST /users', function () {
        ...
        it('Does not create a new user without a strong password', async function () {
            ...
            try {
                ...
                expect(e.response.status).to.equal(422);
                expect(e.response.statusText).to.equal('Unprocessable Entity');
                expect(e.response.data.errors.password.message).to.be.equal('Must contain a combination of at least 1 lowercase letter, 1 uppercase letter, 1 number, 1 special character (!@#$%^&*), and be at least 8 characters long');
            }
        });
