...
function upload(req, res, next) {
    ...
    busboyConfigured.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        if (['image/jpeg', 'image/png'].includes(mimetype)) {
            ...
            fs.ensureDir(dirPath).then(() => {
                file.mv(filePath, async function (err) {
                    ...
                    if (albumId) {
                        PhotoAlbum.findOneAndUpdate({ _id: req.body.albumId }, { $push: { photos: photo } }, (err) => {
                            if (err)
                                return next(err);
                        });
...
async function list(req, res, next) {
    ...
    const photos = Photo.findPaginated({ userId: res.locals.authorizedData.userId }, { skip: +skip, limit: +limit, sort: '-createdAt' });
    ...
    Photo.findByIdAndDelete(req.params.id).then((photo) => {
        if (photo) {
            fs.unlink(`${__dirname}/../../public/images/${res.locals.authorizedData.userId}/${photo.filename}`, (err) => {
                if (err)
                    next(err);
                fs.unlink(`${__dirname}/../../public/images/${res.locals.authorizedData.userId}/thumbs/${photo.filename}`, (err) => {
                    if (err)
                        next(err);
                    PhotoAlbum.updateMany({}, { $pull: { photos: { _id: photo._id } } }).then((_data) => {
